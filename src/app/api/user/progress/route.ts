...
async function buildProgressForFormation(userId: string, formationId: string) {
  ...
  // Statut par module basé sur ItemProgress si présent, sinon fallback
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Récupérer toutes les formations où l'utilisateur est inscrit
    const enrollments = await prisma.enrollment.findMany({ where: { userId: auth.user.id } });
    const results: any[] = [];
    for (const e of enrollments) {
      const p = await buildProgressForFormation(auth.user.id, e.formationId);
      if (p) results.push(p);
    }
